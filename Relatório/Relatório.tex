...
\resume{Neste relatório foi tratado 3 experimentos com o uso de sensores extensômetros cuja variação de resistência elétrica é dependente da uma deformação mecânica sofrida pelo material em que o sensor está cimentado sobre. No primeiro experimento, foi utilizada uma célula de carga comercial e extraída a função de transferência do sistema. Foi constatado que o comportamento da célula era muito próximo de uma reta ideal ($R^2 = 0.998975$), conforme havia-se esperado. No segundo experimento, uma célula de carga não-comercial foi utilizada. Para esta, foi utilizada uma função de aproximação para o cálculo da função de transferência experimental e comparada com a função de transferência experimental obtida calculando-se o valor de saída de tensão elétrica do sensor para diversas massas suspensas sobre a célula. A fim de determinar a frequência de ressonância do sistema, um experimento em que uma batida era realizada sobre a célula e a frequência fundamental do sinal era considerada como a frequência de ressonância do sistema. O resultado comparado com a simulação computacional realizada no SOLIDWORKS indicou um erro de menos de $3 Hz$ do valor experimental para o valor simulado. Também foi feita uma análise do efeito térmico sobre o sensor extensômetro, onde uma aquisição de 4 minutos foi realizada para observar o comportamento de \textit{drift} térmico da estrutura, isto é, há uma pequena e lenta variação de tensão elétrica de saída do sistema com o aquecimento gradual. Por fim, foi utilizado um torquímetro comercial com extensômetros cimentados sobre o teu eixo de torque e a sua função de transferência experimental foi extraída e comparada com estimativas teóricas pré-calculadas.}
\abstract{In this report it was treated three experiments using strain gages sensors whose electrical resistance change is dependent on the mechanical deformation experienced by the material in which the sensor is cemented on. In the first experiment, a load cell was used and its transfer function was calculated. It was found that cell's behavior was very close to an ideal line ($R^2 = 0.998975$), as had been expected. In the second experiment, a non-commercial load cell was used. For this, we used an approximation function for estimating the experimental transfer function and compared it with the experimental transfer function obtained by calculating the output voltage value of the sensor for multiple masses suspended on the cell. In order to determine the frequency of the resonance system, an experiment in which the cell was shocked with another piece of metal and the fundamental frequency of the signal was considered as the frequency of resonance for the entire system. The comparison with the computer simulation done with SOLIDWORKS indicated an error of less than $3 Hz$ from the experimental value. Also an analysis was made of the thermal effect on the extensometer sensor where an acquisition 4 minutes was performed to observe the behavior of the structure's thermal drift, i.e. the system's output voltage slowly changes as heat is applied to the entire system. Finally, we used a commercial torque wrench with strain gauges cemented on its torque shaft and its experimental transfer function was extracted and compared to pre-calculated theoretical estimates.} 
\keywords{célula de carga, experimento, extensômetro, ponte de Wheatstone, strain gage, torquímetro}
...
\includepdf[fitpaper=true,pages=1,pagecommand={
	\pagestyle{fancy}
	\chapter*{Anexo A}
	\label{attachment:a}
}]{../Resources/Mathematica/Aquisicao.nb.pdf}
\includepdf[fitpaper=true,pages=2-,pagecommand={
	\pagestyle{fancy}
}]{../Resources/Mathematica/Aquisicao.nb.pdf}
...
\includepdf[fitpaper=true,pages=1,pagecommand={
	...
	\chapter*{Anexo B}
	\label{attachment:b}
}]{../Resources/Mathematica/Torquimetro.nb.pdf}
